...
def cal_inc():
       ...
       for t1 in range(t1,64):
               if t1 >0 :
                   wlabel=Label(win, text=t1)
                   wlabel.pack(side=RIGHT)
                   t1+=1

               else:
                    wlabel.config(text="enter the number above 1")
                    wlabel.pack()
                    break
...
def cal_dec():
       ...
       for t1 in range(1,t1):
               if t1 >0 :
                   n_Prime=Label(win, text=t1)
                   n_Prime.pack(side=RIGHT)
                   t1=-1
               else:
                    wlabel.config(text="enter the number above 1")
...
def show():
    ...
    for i in options:
        if  clicked.get() == 'Incerement':
                ...
                myButton1['state'] = DISABLED
                mylabel = Label(win, text="Volume DECREMENT")
